...
lvim.builtin.which_key.on_config_done = function()
  ...
  map('n', '<leader>fs', function() builtin.find_files({ previewer = false, search_file = vim.fn.expand('<cword>') }) end
    , { desc = "Find Files String (Current Word under Cursor)" })
  ...
  map('n', '<leader>fn',
    function() builtin.find_files({ prompt_title = ".config/nvim",
  ...
  map('n', '<leader>grn',
    function() builtin.live_grep({ search_dirs = { "$HOME/.config/nvim", "$HOME/.local/share/nvim/site/pack/packer" } }) end
    , { desc = "Grep Neovim Files" })
  ...
  local function set_python_args(args)
    vim.g.python_debug_args = split_by_space(args)
    dap.configurations.python[1].args = vim.g.python_debug_args
  end

  local function set_python_args_from_input()
  ...
  map('n', '<leader>dap', set_python_args_from_input, { desc = "Set Python Arguments" })
  ...
  map('n', '<leader>de', end_debug_session, { desc = "End Debugger" })
